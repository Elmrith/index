...
if ($method === 'POST') {
    ...
    try {
        ...
        $productStmt = $pdo->prepare('SELECT id, name, price, stockQuantity, supplierId, imageUrl FROM products WHERE id = ? AND status = \'approved\' FOR UPDATE');
        ...
        $itemStmt = $pdo->prepare('INSERT INTO order_items (orderId, productId, name, priceAtOrder, quantity, imageUrl) VALUES (?, ?, ?, ?, ?, ?)');
        $itemStmt->execute([$orderId, $productId, $product['name'], $product['price'], $quantity, $product['imageUrl']]);

        $newStock = (int)$product['stockQuantity'] - $quantity;
        $notificationStmt = $pdo->prepare('INSERT INTO notifications (userId, title, message, type, createdAt) VALUES (?, ?, ?, ?, NOW())');
        $notificationStmt->execute([
            $product['supplierId'],
            "New Order Received #{$nextOrderNum} for {$product['name']}",
        ...
        if ((int)$product['stockQuantity'] >= 10 && $newStock < 10) {
            $notificationStmt->execute([
                ...
                "Stock for {$product['name']} is low ({$newStock} units).",
                'inventory_alert'
            ]);
        }
...
if ($method === 'PUT') {
    ...
    if ($userRole !== 'admin' && $userRole !== 'supplier') {
        http_response_code(403);
        echo json_encode(['error' => 'Only suppliers may update orders.']);
        exit;
    }
    ...
    try {
        ...
        if ($userRole !== 'admin' && (string)$order['supplierId'] !== (string)$userId) {
        ...
        if ($newStatus === 'cancelled') {
            $itemStmt = $pdo->prepare('SELECT productId, quantity FROM order_items WHERE orderId = ?');
        ...
        $notifStmt->execute([
            $order['vendorId'],
            "Order #{$order['supplierOrderNumber']} Updated",
            'Your order status is now: ' . ucfirst($newStatus)
        ]);
        $pdo->commit();
